...
class DocsAnalyzer {
     ...
     */
    async analyzeDocumentation(pluginPath) {
     ...
     * Find all documentation files
     */
    findDocumentationFiles(pluginPath) {
        const files = [];
     ...
     */
    analyzeDocFile(filePath, pluginPath) {
     ...
     * Analyze source file documentation
     */
    analyzeSourceDocumentation(filePath, pluginPath) {
        const content = fs.readFileSync(filePath, 'utf8');
        const relativePath = path.relative(pluginPath, filePath);
        const extension = path.extname(filePath);
        const result = {
     ...
     */
    analyzeMarkdownFile(content, result) {
     ...
     * Analyze text file
     */
    analyzeTextFile(content, result) {
        const lines = content.split('\n');
     ...
     * Analyze source file documentation
     */
    analyzeSourceFileDocumentation(content, result) {
        const lines = content.split('\n');
    ...
    outputResults(pluginPath) {
        ...
        const highSeverityIssues = this.results.reduce((sum, r) => sum + r.issues.filter(i => i.severity === 'high').length, 0);
        console.log(`📊 Summary:`);
